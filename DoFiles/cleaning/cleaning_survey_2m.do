...
program drop _all
do "$directorio\DoFiles\cleaning\rename_opm.do"
...
forvalues t = 1/4 {	
	...
	*Homologation
	...
	save  "$directorio\_aux\transferencia_`t'.dta", replace
	
	}
	
	
use "$directorio\_aux\transferencia_1.dta", clear
forvalues t = 2/4 {	
	append using "$directorio\_aux\transferencia_`t'.dta"
	}	
	
merge 1:1 id_opm using `temp_asignaciones', keep(3) nogen
save "$directorio\_aux\opm.dta", replace
 


**************************************GFORMS************************************
insheet using "$directorio\Raw\gf2m.csv", clear
...
save "$directorio\_aux\survey_gf_2m.dta", replace
...
import delimited "$directorio\_aux\survey_data_2m.csv", clear
...
*Combine datasets
append using "$directorio\_aux\survey_gf_2m.dta"
append using "$directorio\_aux\opm.dta"
